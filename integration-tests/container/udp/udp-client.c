...
void mmsghdr_free(struct mmsghdr* mmh, unsigned int len) {
  if (mmh == NULL) {
    return;
  }

  for (int i = 0; i < len * MMSGHDR_N; i++) {
...
struct mmsghdr* mmsghdr_new(target_array_t* targets) {
  assert(targets != NULL);

  struct mmsghdr* mmh = calloc(targets->len * MMSGHDR_N, sizeof(struct mmsghdr));
  if (mmh == NULL) {
    return NULL;
  }
  ...
  for (int i = 0; i < targets->len; i++) {
    ...
    for (int j = 0; j < MMSGHDR_N; j++) {
      struct msghdr* mh = &mmh[j + (i * MMSGHDR_N)].msg_hdr;
...
ssize_t send_sendmmsg(run_data_t* data) {
  ...
  unsigned int vlen = data->targets.len * MMSGHDR_N;
  struct mmsghdr* mmh = mmsghdr_new(&data->targets);
  if (mmh == NULL) {
    return ENOMEM;
  }
  ...
  mmsghdr_free(mmh, data->targets.len);
  return err;
}
...
int main(int argc, char* argv[]) {
  struct option options[] = {
      ...
      {.name = "period", .has_arg = no_argument, .flag = NULL, .val = 'p'},
